...
const UpdateAvatarPage = () => {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      formData.append("avatar", data.avatar[0]); // Only one file

      const res = await axios.patch("/user/update-avatar", formData, {
        headers: {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#0f0f0f] px-4 py-10 text-white">
      ...
      >
        ...
        {/* File Input */}
        <div>
          <label className="block text-sm mb-1">Select New Avatar</label>
          <input
            type="file"
            accept="image/*"
